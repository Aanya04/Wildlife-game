...
function setup() {
  ...
  monster = createSprite(width-200,height/2);
...
function draw() {
    ...
    if(gameState == "level2Finish")
  {
    ...
    {
      if(bulletsGroup[i].isTouching(monster))
      {
        bulletsGroup[i].destroy();
        monsterHits+=1;
        monster.y = random(height/2-200,height/2+200);
        
      }
    }
    if(monster.isTouching(gun))
    {
      gameState = "gameOver"
    }
    if(monsterHits == 30)
    ...
    if(gameState=="won")
    {
      ...
      textSize(70);
        ...
        text("YOU HAVE WON THE GAME",width/2,350)
        textSize(50);
...
function createBullets()
{
  if(frameCount%10 == 0)
  {
    bullet = createSprite(gun.x,gun.y-15);
